...
//#ifdef USERPROG
    ...
    list_push_back(&(running_thread()->child_list),&(t->ch_elem));
//#endif
}
/*
struct file_desc
...
struct thread *find_child(tid_t id)
{
...
struct file_desc *find_filedesc(int fd)
{
    ...
    for( e = list_begin(&current->fd_list); e != list_end(&current->fd_list); e = list_next(e)){
