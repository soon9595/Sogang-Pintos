...
{
    int prevbc = bitmap_size(b);
    b->bit_cnt = b->bit_cnt+size;
    b->bits = (elem_type*)realloc(b->bits,sizeof(unsigned long)*(b->bit_cnt));
    if(b->bits!=NULL||(int)b->bit_cnt!=0)
    {
        bitmap_set_multiple(b,prevbc,size,false);
        return b;
    }
    else{
        printf("Failure!\n");
