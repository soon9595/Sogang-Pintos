...
int syscall_read(int fd,void *buffer, unsigned size)
{
    ...
    if(fd==0)
    {
        ...
        if (is_kernel_vaddr(buffer) 
                || !pagedir_get_page (cur->pagedir, buffer)){
            syscall_exit (-1);
        }
        ...
        //    lock_acquire (&f_lock);
            number_bytes = file_read (fp, buffer, size);
            lock_release (&f_lock);
...
syscall_write(int fd,const void *buffer,unsigned size)
{
    ...
    else if( fd ==0)
    {
        ...
        //         lock_acquire (&f_lock);
                 number_bytes = file_write (fp, buffer, size);
                 lock_release (&f_lock);
                 return number_bytes;
             }
         }
...
void syscall_exit(int status)// typedef int pid_t
{
    ...
    sema_down(&thread_dead->p_wait_sema);
    sema_up(&thread_dead->parent->wait_sema);
    thread_dead->parent->child_dead_status = status;
    thread_dead->parent->dead_status = true;
    thread_exit();
}
